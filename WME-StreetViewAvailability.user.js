...
(function() {
  ...
  function init(e) {
    ...
    if (displayGroupToggle != null) {
      ...
      displayGroupToggle.addEventListener('change', function() {
        checkbox.disabled = !displayGroupToggle.checked;
    ...
    var controlObserver = new MutationObserver(function(mutationRecords) {
      if (mutationRecords[mutationRecords.length-1].target.style.display == 'none' && displayGroupToggle.checked) {
    ...
    // Restore the previous width when opening Street View Panorama, if stored
    W.streetViewController.on('stateChanged', (visible) => {
      if (visible && localStorage.getItem('WMEStreetViewWidth')) {
        document.getElementById('street-view-container').style.width = 'calc(100% - ' + localStorage.getItem('WMEStreetViewWidth') + ')';
        document.getElementById('street-view-drag-handle').style.left = localStorage.getItem('WMEStreetViewWidth');
        document.getElementById('waze-map-container').style.width = localStorage.getItem('WMEStreetViewWidth');
        W.streetViewController.trigger('resizeStreetView');
      }
    });

    // Listen to changes in the width of the Street View container div to restore the width afterwards
    var wazeMapContainer = document.getElementById('waze-map-container');
    var containerObserver = new MutationObserver((mutations) => {
      var newContainerWidth = wazeMapContainer.style.width;
      if (newContainerWidth.length > 0) {
        localStorage.setItem('WMEStreetViewWidth', wazeMapContainer.style.width);
      }
    });
    containerObserver.observe(wazeMapContainer, {
      attributes: true,
