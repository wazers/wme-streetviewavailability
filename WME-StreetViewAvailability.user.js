...
(function() {
  ...
  function init(e) {
    ...
    if (streetViewControl === null) {
      setTimeout(init, 400);
      console.log('Street view elements unavailable, retrying in 400ms');
    }
    ...
    var controlObserver = new MutationObserver(function(mutationRecords) {
      try {
        if (isHidden(mutationRecords[0].target) != pinWasHidden) {
          if (pinWasHidden == true && displayGroupToggle.checked) {
            pinWasHidden = isHidden(mutationRecords[0].target);
    ...
    controlObserver.observe(streetViewControl, { attributes: true, attributeFilter: ['style', 'class'] });
